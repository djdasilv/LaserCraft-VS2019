...
int main(int argc, char* argv[])
{
    ...
    camera->set(cVector3d(0.05, 0.0, 0.02),    // camera position (eye)
    ...
    // create small sphere cursor
    cursor = new cShapeSphere(0.0003);

    // add cursor to world
    world->addChild(cursor);

    // set cursor color
    cursor->m_material->setBlueCornflower();
    ...
    object->m_minCorner.set(-0.01, -0.01, -0.01);
    object->m_maxCorner.set(0.01, 0.01, 0.01);
...
void updateGraphics(void)
{
    ...
    cursor->setLocalPos(robotPosCur - offset);
